...
try
{
    ...
    if ( Os.isName( "Linux" ) )
    {
        artifact  = new File( out, "jpackager-its-jvm-args-99.0-1.x86_64.rpm" );
        if ( !artifact.exists() || artifact.isDirectory() )
        {
            System.err.println( "jpackager-its-jvm-args-99.0-1.x86_64.rpm file is missing or is a directory." );
            return false;
        }
    ...
    {
        artifact  = new File( out, "jpackager-its-jvm-args-99.0.dmg" );
        if ( !artifact.exists() || artifact.isDirectory() )
        {
            System.err.println( "jpackager-its-jvm-args-99.0.dmg file is missing or is a directory." );
            return false;
        }
    }
}
